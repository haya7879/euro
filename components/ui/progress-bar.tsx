 ...
 * Accessible progress indicator with ARIA progressbar role
...
export default function ProgressBar({ 
  ...
  return (
    <div 
      ...
      role="status"
      aria-live="polite"
      aria-label={isComplete ? 'Download complete' : `Downloading: ${progressValue}% complete`}
    >
      ...
      <div className="relative">
        <div 
          className="w-full bg-gray-200 rounded-full h-1.5 sm:h-2 overflow-hidden"
          role="progressbar"
        ...
        >
          ...
          >
            {/* Shimmer effect */}
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent animate-pulse" aria-hidden="true" />
          </div>
        </div>
      </div>
      <span className="sr-only">
        {isComplete ? 'Download completed successfully' : `Downloading, ${progressValue} percent complete`}
      </span>
    </div>
  )
}
